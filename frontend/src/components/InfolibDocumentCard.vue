<template>
  ...
  <div class="flex-shrink-0 p-0.5">
    <img class="h-32 w-full object-cover object-top rounded-lg rounded-b-none border-b-2 opacity-50" :src="previewUrl" alt="" v-if="previewUrl" />
  </div>
    ...
    <div class="flex-1">
      ...
      <p class="text-base font-medium text-aic-primary">
        ...
        <span v-for="(topic, idx) of d.topics">
          {{ topic }}{{ d.topics.length > 1 && idx < d.topics.length-1 ? ', ' : '' }}
        </span>
      </p>
      <a class="block mt-2">
...
const previewUrl = computed(() => {
  return "/infolib/previews/brochures/"+props.d.fileId+".jpg";
});
